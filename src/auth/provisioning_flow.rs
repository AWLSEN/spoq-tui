...
/// Prompt the user to select a plan by number.
///
/// # Arguments
/// * `max` - The maximum valid selection (number of plans)
///
/// # Returns
/// * `Ok(usize)` - The zero-indexed selection
/// * `Err(CentralApiError)` - Input error occurred
pub fn prompt_plan_selection(max: usize) -> Result<usize, CentralApiError> {
    loop {
...
///
/// # Returns
/// * `Ok(String)` - The entered password
/// * `Err(CentralApiError)` - Input error occurred
pub fn prompt_password() -> Result<String, CentralApiError> {
    print!("Enter password for VPS: ");
    io::stdout().flush().map_err(|e| CentralApiError::ServerError {
        status: 0,
        message: format!("Failed to flush stdout: {}", e),
    })?;

    rpassword::read_password().map_err(|e| CentralApiError::ServerError {
        status: 0,
        message: format!("Failed to read password: {}", e),
    })
}

/// Poll the VPS status until it's ready or times out.
///
/// # Arguments
/// * `runtime` - The Tokio runtime to use for async operations
...
/// * `Err(CentralApiError)` - Polling failed or timed out
pub fn poll_vps_status(
    runtime: &tokio::runtime::Runtime,
