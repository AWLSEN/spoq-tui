...
impl<'a> TextArea<'a> {
    ...
    /// Enable or disable soft line wrapping. When enabled, long lines are wrapped visually
    /// to fit within the widget width. The underlying text content is not modified.
    /// By default, line wrapping is disabled.
    /// ```
    /// use tui_textarea::TextArea;
    ///
    /// let mut textarea = TextArea::default();
    /// textarea.set_line_wrap(true);
    /// assert!(textarea.line_wrap());
    /// ```
    pub fn set_line_wrap(&mut self, wrap: bool) {
        self.line_wrap = wrap;
    }

    /// Get whether soft line wrapping is enabled.
    /// ```
    /// use tui_textarea::TextArea;
    ///
    /// let textarea = TextArea::default();
    /// assert!(!textarea.line_wrap()); // Disabled by default
    /// ```
